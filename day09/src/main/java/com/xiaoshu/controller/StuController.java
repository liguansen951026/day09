...
public class StuController extends LogController{
	...
	// 新增专业
		...
		public void addMajor(HttpServletRequest request,Major major,HttpServletResponse response){
			...
			try {
				...
				final String maj = JSON.toJSONString(major);
				...
				jt.send(itemcat_queue,new MessageCreator() {
					...
					public Message createMessage(Session session) throws JMSException {
						...
						TextMessage textMessage = session.createTextMessage(maj);
		...
		try {
			if(file != null){
				...
				for (int i = 1; i <= lastRowNum; i++) {
					...
					Date date = simpleDateFormat.parse(row.getCell(4).getStringCellValue());
					student.setSdbirth(date);
					String mdname = row.getCell(5).getStringCellValue();
					if("大数据".equals(mdname)){
						student.setMid(1);
					}else if("全栈".equals(mdname)){
						student.setMid(2);
					}else{
						student.setMid(3);
					}
	...
	private void ExportExcelToDisk(HttpServletRequest request,
		...
		try {
			...
			for (int i = 0;i < list.size(); i++) {
			    ...
			    if("大数据".equals(mdname) && sdhobby.contains("LOL")){
	...
	public void findAddr(Integer id,HttpServletRequest request,HttpServletResponse response) throws Exception{
		try {
		
			List<Address> list = ss.findAllAddress(id);
	...
	public String index(HttpServletRequest request,Integer menuid) throws Exception{
		List<Major> majorList = ss.findAllMajor();
		List<Operation> operationList = operationService.findOperationIdsByMenuid(menuid);
		List<Address> plist = ss.findAllAddress(0);
	...
	public void userList(Student stu,HttpServletRequest request,HttpServletResponse response,String offset,String limit) throws Exception{
		try {
			...
			PageInfo<Student> stuList= ss.findStuPage(stu,pageNum,pageSize);
		...
		try {
			if (sdId != null) {   // userId不为空 说明是修改
				...
				if(student==null){
					ss.updateStu(stu);
					result.put("success", true);
				}else if(student != null && student.getSdid().compareTo(sdId)==0){
					ss.updateStu(stu);
					result.put("success", true);
				}else{
					result.put("success", true);
